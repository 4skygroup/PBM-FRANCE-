import RealisationCard from "./RealisationCard";
import realisations from "../../data/Realisations.js";

export default function ListeRealisations() {
    return (
        <section id="realisations" className="px-4 py-20 md:px-[60px] md:py-[120px] bg-pbm-noir">

            {/* Section tag */}
            <div className="flex items-center gap-2.5 mb-4">
                <span className="w-6 h-px bg-pbm-blue3" />
                <span className="text-[10px] text-pbm-blue3 uppercase tracking-[3px] font-semibold font-dm">
                  Portfolio
                </span>
            </div>

            {/* Titre */}
            <h2 className="font-bebas text-[clamp(42px,5vw,72px)] leading-[.95] tracking-tight mb-4 text-pbm-white">
                Nos{" "}
                <span
                    style={{
                        background: "linear-gradient(90deg, #4f46e5, #6366f1)",
                        WebkitBackgroundClip: "text",
                        WebkitTextFillColor: "transparent",
                    }}
                        >
                  réalisations
                </span>
            </h2>

            {/* Description */}
            <p className="text-[15px] text-pbm-grey2 max-w-[480px] leading-[1.7] font-light mb-12">
                Quelques exemples de chantiers réalisés en Île-de-France. Chaque projet est unique.
            </p>

            {/* Grille mobile : 1 col, toutes les cards à hauteur fixe
          Desktop : 3 col, 2 rows, première card span 2 lignes */}
            <div className="
                grid gap-3
                grid-cols-1
                auto-rows-[220px]
                md:grid-cols-[2fr_1fr_1fr]
                md:grid-rows-[240px_240px]
                md:auto-rows-[unset]
            ">
                {realisations.map((item) => (
                    <RealisationCard
                        key={item.id}
                        label={item.label}
                        lieu={item.lieu}
                        image={item.image}
                        large={item.large}
                    />
                ))}
            </div>

        </section>
    );
}